...
-- SETTINGS
------------------------------------------------
local ANIM_TRIGGER = "1461157246"  -- press 2, or 3 if Chop-Chop Cannon is on cooldown
local ANIM_START   = "1461136875"  -- start move -> press 1
...
-- Returns true if the item exists and is on cooldown.
local function isOnCooldown(itemName)
	local item = getBackpackItem(itemName)
	if not item then return false end
	local cooldown = item:GetAttribute("COOLDOWN")
	return cooldown ~= nil and cooldown ~= COOLDOWN_READY
end
...
local function onAnimationPlayed(track)
	...
	-- 1461157246: press 2, or 3 if Chop-Chop Cannon is on cooldown.
	if numericId == ANIM_TRIGGER then
		if isOnCooldown("Chop-Chop Cannon") then
	...
	-- The rest only counts while the damage-detection window is armed.
