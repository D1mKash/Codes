...
local a={["1461157246"]={{key=Enum.KeyCode.Three,delay=0},{key=Enum.KeyCode.Q,delay=0.01}}}
...
local function i()local s,e=u.InputBegan:Connect(function(input,gp)if not r or gp then return end if input.KeyCode==Enum.KeyCode.Two then task.spawn(function()task.wait(0.1)k(Enum.KeyCode.One)v:SendMouseButtonEvent(0,0,true,game)task.wait(0.01)v:SendMouseButtonEvent(0,0,false,game)end)end end) table.insert(c,e)end
